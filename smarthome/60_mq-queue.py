...
cmd='''curl http://%s:8161/admin/queues.jsp 2>/dev/null |grep JMSDestination=Consumer | awk -F "?" '{print $2}' |sed "s/\\">.*//g" | sed "s/&.*//g"|sort -u | sed "s/JMSDestination=//g"''' %ip_list.split('\n')[0]

mq_queue=commands.getoutput(cmd)
mq_queue=mq_queue.split('\n')

for i in mq_queue:
	cmd='''curl http://%s:8161/admin/queues.jsp 2>/dev/null |grep -m 2 -A 2 %s | egrep -m 1 "<td>[0-9]+" | awk -F "<|>" '{print $3}' ''' %(ip_list.split('\n')[0],i)
