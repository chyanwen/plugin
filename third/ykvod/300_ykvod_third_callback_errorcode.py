...
with open(logfile,'r') as logfile_fd:
    ...
    for line in logfile_fd:
        if 'youku return' in line:
            errorcode=int(line.split(":")[3].split(',')[0])
            order_id=line.split()[1]
            if errorcode!=1:
                counter_list.append({"endpoint":endpoint,"metric":'third.callback.errcode',"tags":'third=ykvod,order_id=%s' %order_id,"timestamp":ts,"step":step,"counterType":"GAUGE","value":errorcode})
