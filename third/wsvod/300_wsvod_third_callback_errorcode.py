...
with open(logfile,'r') as logfile_fd:
    ...
    for line in logfile_fd:
        if 'aiyi return:' in line:
            errorcode=int(line.split(':')[5].split(",")[0])
            order_id=line.split()[1]
            if errorcode!=200:
                counter_list.append({"endpoint":endpoint,"metric":'third.callback.errcode',"tags":'third=aiyi,order_id=%s' %order_id,"timestamp":ts,"step":step,"counterType":"GAUGE","value":errorcode})
        elif 'huasu return result' in line:
            errorcode=int(line.split(':')[3].split(',')[0])
            order_id=line.split()[0].split(':')[1]
            if errorcode!=0: 
                counter_list.append({"endpoint":endpoint,"metric":'third.callback.errcode',"tags":'third=huasu,order_id=%s' %order_id,"timestamp":ts,"step":step,"counterType":"GAUGE","value":errorcode})
